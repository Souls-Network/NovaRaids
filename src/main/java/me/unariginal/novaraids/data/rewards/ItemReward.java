...
public class ItemReward extends Reward {
    ...
    public void applyReward(ServerPlayerEntity player) {
        ...
        // Try to place the reward in the player's inventory first.
        boolean fullyInserted = player.getInventory().insertStack(reward);

        // If the inventory was full, overflow into the player's ender chest.
        if (!fullyInserted && !reward.isEmpty()) {
            EnderChestInventory enderChest = player.getEnderChestInventory();
            ItemStack leftover = enderChest.addStack(reward);

            // If the ender chest is also full, drop whatever is left on the ground.
            if (!leftover.isEmpty()) {
