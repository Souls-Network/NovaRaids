...
import me.unariginal.novaraids.commands.suggestions.LocationSuggestions;
...
public class RaidCommands {
                ...
                for (ServerPlayerEntity player : playersToReward) {
                    if (player != null) {
                        ...
                        if (!noMoreRewards.containsKey(player) || (duplicatePlacementExists && place.place().equalsIgnoreCase(noMoreRewards.get(player)))) {
                            ...
                            Map<RewardPool, Double> availablePools = new HashMap<>(reward.pools());
    ...
    public static int start(Boss bossInfo, @Nullable String location, ServerPlayerEntity player, ItemStack startingItem) {
        if (NovaRaids.LOADED && (!nr.server().getPlayerManager().getPlayerList().isEmpty() || RaidCommands.nr.config().runRaidsWithNoPlayers)) {
            if (bossInfo != null) {
                // Participation limit: cannot host while already in another raid (vouchersJoinRaids
                // would otherwise start a second raid and fail auto-join — still leaves a orphan host raid).
                if (player != null
                        && !NovaRaidsPermissions.OVERRIDE.test(player)
                        && nr.isPlayerParticipating(player.getUuid())) {
                    player.sendMessage(TextUtils.deserialize(TextUtils.parse(
                            nr.messagesConfig().getMessage("warning_already_in_raid_cannot_host"), bossInfo)));
                    return 0;
                }
                // Bind player hosts to the raid area they are standing in (1 active raid per area).
                // Boss configs often list only example_location; without this, a second host always fails.
                boolean boundToPlayerArea = false;
                if (location == null && player != null) {
                    Location nearest = nr.locationsConfig().getNearestLocation(player);
                    if (nearest != null) {
                        location = nearest.id();
                        boundToPlayerArea = true;
                        nr.logInfo("Hosting at nearest raid area: " + location);
                    }
                }

                Map.Entry<?, Double> spawnLocationEntry;
                HashMap<String, Double> validLocations = new HashMap<String, Double>();
                if (location == null) {
                    Map<String, Double> spawnLocations = bossInfo.spawnLocations();
                    for (String key : spawnLocations.keySet()) {
                        if (nr.locationsConfig().getLocation(key) == null) continue;
                        boolean validSpawn = true;
                        for (Raid raid : nr.activeRaids().values()) {
                            if (!raid.raidBossLocation().id().equalsIgnoreCase(key)) continue;
                            validSpawn = false;
                            break;
                        }
                    ...
                    boolean validSpawn = true;
                    for (Raid raid : nr.activeRaids().values()) {
                        if (!raid.raidBossLocation().id().equalsIgnoreCase(location)) continue;
                        validSpawn = false;
                        break;
                    }
                    if (validSpawn) {
                ...
                if (!validLocations.isEmpty()) {
                    spawnLocationEntry = RandomUtils.getRandomEntry(validLocations);
                    ...
                    if (player != null) {
                        String messageKey = boundToPlayerArea ? "location_busy" : "no_available_locations";
                        player.sendMessage(TextUtils.deserialize(TextUtils.parse(nr.messagesConfig().getMessage(messageKey), bossInfo)));
                    }
                    return 0;
                }
                String spawnLocationString = (String)spawnLocationEntry.getKey();
                Location spawnLocation = nr.locationsConfig().getLocation(spawnLocationString);
                if (spawnLocation != null) {
                    // Location is already free — start immediately. Queue only when use_queue_system
                    // is on AND another raid is already using this same area (should not happen here).
                    if (!RaidCommands.nr.config().useQueueSystem || !isLocationBusy(spawnLocation.id())) {
                        nr.logInfo("Starting raid at " + spawnLocation.id() + ".");
                        nr.addRaid(new Raid(bossInfo, spawnLocation, player != null ? player.getUuid() : null, startingItem));
                    } else {
                        nr.logInfo("Adding queue raid for busy location " + spawnLocation.id() + ".");
    ...
    private static boolean isLocationBusy(String locationId) {
        for (Raid raid : nr.activeRaids().values()) {
            if (raid.raidBossLocation().id().equalsIgnoreCase(locationId)) {
                return true;
            }
        }
        return false;
