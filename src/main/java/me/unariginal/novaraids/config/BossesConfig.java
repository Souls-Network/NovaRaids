...
public class BossesConfig {
    ...
    public void loadBoss(String category_name, File file) throws IOException, NullPointerException, UnsupportedOperationException {
        ...
        List<String> onDefeat = this.readStringList(root, "onDefeat");
        List<String> onVictory = this.readStringList(root, "onVictory");
        List<String> onError = this.readStringList(root, "onError");
    ...
    private List<String> readStringList(JsonObject node, String name) {
        JsonElement json;
        if (node.has(name) && (json = node.get(name)).isJsonArray()) {
            return json.getAsJsonArray().asList().stream().filter(JsonElement::isJsonPrimitive).map(JsonElement::getAsJsonPrimitive).filter(JsonPrimitive::isString).map(JsonPrimitive::getAsString).toList();
        }
        return List.of();
    }
}
