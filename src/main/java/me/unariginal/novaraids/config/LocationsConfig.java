...
public class LocationsConfig {
    ...
    /**
     * Nearest configured raid location in the player's world, within host range.
     * Used so hosting binds to the area the player is standing in (1 raid per area).
     */
    public Location getNearestLocation(ServerPlayerEntity player) {
        if (player == null) {
            return null;
        }
        ...
        for (Location loc : locations) {
            if (loc.world() != player.getServerWorld()) {
                continue;
            }
            double hostRange = Math.max(loc.borderRadius() * 2.5, 80.0);
            double distSq = player.getPos().squaredDistanceTo(loc.pos());
            if (distSq <= hostRange * hostRange && distSq < nearestDistSq) {
