    ...
    public static RewardPool getRewardPool(JsonObject rewardPoolObject, String name) {
        ...
        for (JsonElement rewardElement : rewardsArray) {
            ...
            if (rewardsObject.has("reward")) {
                ...
                // Unique names required: allow_duplicates:false tracks applied rewards by name.
                // Using a shared "temp" name made every later roll see zero remaining weight.
                String inlineRewardName = name + "_" + inlineRewardIndex++;
                Reward reward = getReward(rewardObject, inlineRewardName);
