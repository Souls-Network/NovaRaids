...
public class TickManager {
    ...
    /** Spread Discord webhook edits across ticks when many raids are active. */
    private static int webhookRaidCursor = 0;
    private static int catchMaintenanceTick = 0;
    private static int playerPositionTick = 0;
    ...
    public static void updateWebhooks() throws ConcurrentModificationException {
        ...
        List<Raid> webhookRaids = new ArrayList<>();
        for (Raid raid : nr.activeRaids().values()) {
            if ((raid.stage() == 1 || raid.stage() == 2) && raid.getCurrentWebhookID() != 0L) {
                webhookRaids.add(raid);
            }
        }
        if (webhookRaids.isEmpty()) {
            webhookRaidCursor = 0;
            webhookUpdateTimer = WebhookHandler.webhookUpdateRateSeconds * 20;
            return;
        }
        if (webhookRaidCursor >= webhookRaids.size()) {
            webhookRaidCursor = 0;
        }
        Raid raid = webhookRaids.get(webhookRaidCursor++);
        long id = raid.getCurrentWebhookID();
        if (raid.stage() == 1) {
            WebhookHandler.editStartRaidWebhookAsync(id, raid);
        } else {
            WebhookHandler.editRunningWebhookAsync(id, raid);
        }
        if (webhookRaidCursor >= webhookRaids.size()) {
            webhookRaidCursor = 0;
            webhookUpdateTimer = WebhookHandler.webhookUpdateRateSeconds * 20;
    ...
    public static void fixBossPositions() throws ConcurrentModificationException {
        boolean runCatchMaintenance = ++catchMaintenanceTick >= 20;
        if (runCatchMaintenance) {
            catchMaintenanceTick = 0;
        }
        for (Raid raid : nr.activeRaids().values()) {
            raid.fixBossPosition();
            if (raid.stage() == 4 && runCatchMaintenance) {
                for (UUID playerUuid : raid.participatingPlayers()) {
                    ServerPlayerEntity player = nr.server().getPlayerManager().getPlayer(playerUuid);
                    if (player != null && raid.hasCatchAssignment(playerUuid)) {
                        raid.respawnCatchEncounterIfNeeded(player);
                        // Repair: strip leftover catch tags from party Pokemon (older builds).
                        for (Pokemon partyMon : Cobblemon.INSTANCE.getStorage().getParty(player)) {
                            if (partyMon != null) {
                                CatchEncounterTags.stripLeftoverTagsIfPlayerOwned(partyMon);
                            }
                        }
                        PokemonEntity owned = raid.findLiveCatchCloneForPlayer(playerUuid);
                        if (owned != null && !owned.isBattling() && owned.getBeamMode() == 0) {
                            CatchEncounterEntityHelper.clearBusyState(owned);
                        }
                    }
                }
                ...
                for (PokemonEntity pokemonEntity : raid.getClones().keySet()) {
                    UUID playerUUID = raid.getCloneOwnerUuid(pokemonEntity);
                    ServerPlayerEntity player = playerUUID != null
                            ? nr.server().getPlayerManager().getPlayer(playerUUID)
                            : null;
                    if (player != null) {
    ...
    public static void fixPlayerPositions() throws ConcurrentModificationException {
        if (++playerPositionTick < 5) {
            return;
        }
        playerPositionTick = 0;
        for (Raid raid : nr.activeRaids().values()) {
                ...
                if (player != null && player.getServerWorld() == world) {
                        ...
                        if (hyp < raidPushback) {
                            if (raid.stage() == 4 && raid.hasCatchAssignment(player.getUuid())) {
                                // Catch-phase stand spots sit inside the ring on purpose.
                            } else {
    ...
    public static void updateBossbars() throws ConcurrentModificationException {
        boolean refreshActionbar = ++actionbarTick >= 10;
        if (refreshActionbar) {
            actionbarTick = 0;
        }
        for (Raid raid : nr.activeRaids().values()) {
