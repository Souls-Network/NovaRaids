...
public class NovaRaids {
    ...
    /** Catch/battle clone entity UUID → owning player UUID (avoids scanning all raids per visibility check). */
    private final Map<UUID, UUID> catchCloneOwnerByEntityId = new HashMap<>();
    ...
    public NovaRaids(IEventBus bus) {
        ...
        // Clean up at server stop — retire Discord embeds before process exit.
        PlatformEvents.SERVER_STOPPING.subscribe(Priority.NORMAL, server -> {
            if (LOADED) {
                ...
                for (Raid raid : new ArrayList<>(activeRaids.values())) {
                    try {
                        raid.stop();
                    } catch (Exception e) {
                        logError("Error stopping raid during shutdown: " + e.getMessage());
                    }
                }
                activeRaids.clear();
    ...
    public void registerCatchCloneEntity(UUID entityId, UUID ownerUuid) {
        if (entityId != null && ownerUuid != null) {
            catchCloneOwnerByEntityId.put(entityId, ownerUuid);
        }
    }

    public void unregisterCatchCloneEntity(UUID entityId) {
        if (entityId != null) {
            catchCloneOwnerByEntityId.remove(entityId);
        }
    }

    public UUID getCatchCloneOwnerByEntityId(UUID entityId) {
        return entityId != null ? catchCloneOwnerByEntityId.get(entityId) : null;
    }
    ...
    /** True if the player is already in any active raid's participatingPlayers list. */
    public boolean isPlayerParticipating(UUID playerUUID) {
        if (playerUUID == null) {
            return false;
        }
        for (Raid raid : activeRaids.values()) {
